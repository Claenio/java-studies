...
* ### Arrays & LinkedList
  **Arrays** have a fixed size. Once they are created, it is not possible to increase their size directly. If it becomes necessary to store more elements, it is usually required to create a new larger array and copy the old data into it.
  ...
  ####
  Arrays provide very fast access to elements, with a time complexity of O(1), because it is possible to directly access an item through its index. However, insertions and removals can be slow, especially in the middle of the structure, since the following elements need to be shifted, resulting in a complexity of O(n).
  A Linked List, on the other hand, has slower read operations, with a complexity of O(n), because it is necessary to traverse the nodes sequentially until the desired element is found. Unlike arrays, the elements are not stored contiguously in memory, and each node stores a reference to the next one.
  Insertions and removals in a Linked List can be O(1) when we already have a reference to the correct node, since it is only necessary to change the pointers between the nodes. However, if it is necessary to locate the position first, the complexity becomes O(n).
  In addition, arrays are usually faster and more memory-efficient, while Linked Lists offer greater flexibility for dynamic growth. Each structure has its own advantages and disadvantages depending on the problem being solved.